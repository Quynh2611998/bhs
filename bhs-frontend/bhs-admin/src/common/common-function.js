...
export function getRole() {
    const cookies = document.cookie.split(';');
    for (var i = 0; i < cookies.length; i++) {
        if (cookies[i].includes('BHS-AUTHORITY')) {
            const role = cookies[i].split('=')[1];
            if (role.length > 0) {
                return role;
            }
        }
...
export function formatDateAsStringDDMMYYYY(date) {
    ...
    const month = (parseDate.getMonth() + 1) < 10    ? ('0' + (parseDate.getMonth() + 1)) : (parseDate.getMonth() + 1);
    const day   = (parseDate.getDate()) < 10         ? ('0' + parseDate.getDate())        : (parseDate.getDate());
...
export function formatDateTimeAsStringWithMilisec(date) {
    ...
    const mm    = (parseDate.getMonth() + 1) < 10    ? ('0' + (parseDate.getMonth() + 1)) : (parseDate.getMonth() + 1);
    const dd    = (parseDate.getDate())      < 10    ? ('0' + parseDate.getDate())        : (parseDate.getDate());
    const hh    = (parseDate.getHours())     < 10    ? ('0' + parseDate.getHours())       : (parseDate.getHours());
    const mms   = (parseDate.getMinutes())   < 10    ? ('0' + parseDate.getMinutes())     : (parseDate.getMinutes()); 
    const ss    = (parseDate.getSeconds())   < 10    ? ('0' + parseDate.getSeconds())     : (parseDate.getSeconds());
