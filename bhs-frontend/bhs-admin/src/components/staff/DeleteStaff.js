...
function  DeleteStaff({ hide, data, name, setDate }) {

    function HandleDeleteStaff(id) {
        axios.post(SERVER_API_URL + `api/admin/delete-hair-stylist-by-id/${id}`, id).then( res => {
                if(res.status === 200) {
                    setDate()
                }
        })
            ...
            <div className='modal-wrapper w-25'>
                ...
                <div className='modal-body'>
                    ...
                    <p className='fs-5 text'>
                        Do you want to delete staff: {name}  ?
                    </p>
                </div>
                <div className='modal-control mb-4'>
                    <button type='button' className='btn btn-danger mx-4 ' style={{width: "5rem"}}  onClick={() => {HandleDeleteStaff(data);hide()}} >Delete</button>
