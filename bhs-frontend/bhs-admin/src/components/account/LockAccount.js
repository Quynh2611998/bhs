...
const LockAccountModal = ({ isShowing, hide,data,setDate }) => isShowing ? ReactDOM.createPortal(
        ...
        <div className='modal-wrapper w-25'>
            ...
            <div className='modal-body'>
                ...
                <p className='fs-5 text'>
                    Do you want to lock account name: {data.userName} ?
                </p>
            </div>
            <div className='modal-control mb-4'>
                <button type='button' className='btn btn-primary mx-4 ' style={{width: "5rem"}}  onClick={() => {hide();HandleLockAccount(data.id,setDate);}} >Lock</button>
...
export function HandleLockAccount(id,setDate) {
        axios.put(SERVER_API_URL + 'api/admin/lock-account/'+id).then( res => {
            setDate()
        })
}
