...
function DeleteBooking({ hide, setDate, id }) {
    function HandleDeleteOption(id) {
        axios.post(SERVER_API_URL + 'api/delete-booking', [id]).then(res => {
            setDate()
        })
    }
    return (
            ...
            <div className='modal-wrapper w-25'>
                ...
                <div className='modal-body'>
                    ...
                    <p className='fs-5 text'>
                        Do you want to delete booking has id: {id} ?
                    </p>
                </div>
                <div className='modal-control mb-4'>
                    <button type='button' className='btn btn-danger mx-4 ' style={{ width: "5rem" }} onClick={() => { HandleDeleteOption(id); hide() }} >Delete</button>
