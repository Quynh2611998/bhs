...
function DeleteCategory({ hide, setDate, name, id }) {
    function HandleDeleteService(_id) {
        debugger
        axios.post(SERVER_API_URL + `api/admin/delete-category-by-id/${_id}`, [_id]).then(res => {
            if (res.status === 200) {
                setDate()
            }
        })
        ...
        <div className='modal-container'>
            ...
            <div className='modal-wrapper w-25'>
                ...
                <div className='modal-control mb-4'>
                    <button type='button' className='btn btn-danger mx-4 ' style={{ width: "5rem" }} onClick={() => { HandleDeleteService(id); hide() }} >Delete</button>
